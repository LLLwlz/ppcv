...
dlamly = [1, 0.5, 0.1, 0.05, 0.01, 0.005, 0.001, 0.0005, 0.0001, 0.00005, 0.00001, 0.000005, 0.000001, 0.0000005,
          0.0000001]
...
def train():
    ...
    for cs in range(len(model_list_name)):
        for data_index in range(len(dataset_name)):
            ...
            train_logger = LogWay('logger/' + datetime.datetime.now().strftime(
                '%Y-%m-%d %H-%M-%S') + model_name + '_' + data_name + '.txt')
            ...
            for epoch in range(opt.epoch):
                ...
                realepoch = epoch + 1
                if (realepoch % 1 == 0):
                    info = 'Time:{} dataset:{} Epoch:{} Loss_avg:{}\n'.format(str(datetime.datetime.now()), data_name,
                                                                              epoch + 1, loss_sum / (i + 1))
                    train_logger.add(info)
                adjusting_rate(optimizer, opt.learning_rate, epoch + 1)
                if (realepoch % 1 == 0):
                    save_name = 'save_model/' + datetime.datetime.now().strftime(
                        '%Y-%m-%d %H-%M-%S') + ' ' + model_name + '_' + data_name + str(realepoch) + '.pt'
